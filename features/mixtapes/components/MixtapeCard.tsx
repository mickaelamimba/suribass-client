...
export function MixtapeCard({ mixtape }: MixtapeCardProps) {
    ...
    <>
      <div className="group relative flex flex-col overflow-hidden rounded-xl border bg-card transition-all hover:shadow-lg">
        ...
        <div className="relative aspect-square w-full overflow-hidden bg-muted">
          ...
          {/* Score Badge (if high) */}
          {mixtape.score !== null && (
            typeof mixtape.score === 'number' 
              ? mixtape.score >= 80 
              : mixtape.score.score >= 80
          ) && (
            <div className="absolute top-2 left-2">
              <Badge variant="secondary" className="bg-yellow-500/90 text-white hover:bg-yellow-500">
                ★ {typeof mixtape.score === 'number' ? mixtape.score : mixtape.score.score}
              </Badge>
            </div>
          )}
        </div>
